...
from d2.utils.logger import _log_api_usage
...
class SimpleTrainer(TrainerBase):
    ...
    def next_data(self):
        try:
            ...
            if self.data_sampler is not None:
                try:
                    self.data_sampler.set_epoch(self.iter)
                except:
                    logger.info("Sampler " + str(self.data_sampler) + " do not have a set_epoch method.")
            else:
                raise NotImplementedError("Dataset should either be a Infinite Iterable Dataset or Use a sampler")
            self._data_loader_iter = iter(self.data_loader)
            return next(self._data_loader_iter)
    ...
    def run_step(self):
        ...
        if not self.model.training:
            logger.warning("[SimpleTrainer] model was changed to eval mode!")
            self.model.train()
            logger.warning("Set it back to training mode.")
...
class AMPTrainer(SimpleTrainer):
    ...
    def __init__(
        ...
        super().__init__(
            model, data_loader, optimizer, data_sampler, 
            gather_metric_period, zero_grad_before_forward, async_write_metrics
        )
