...
def axis_angle_to_matrix_np(axis_angle):
    return quaternion_to_matrix(
        axis_angle_to_quaternion(torch.from_numpy(axis_angle))
    ).numpy()
...
def matrix_to_rotation_6d_np(matrix):
    return matrix_to_rotation_6d(torch.from_numpy(matrix)).numpy()

def axis_angle_to_rotation_6d(axis_angle):
    matrix = quaternion_to_matrix(axis_angle_to_quaternion(axis_angle))
...
def rotation_6d_to_matrix_np(x):
    x = torch.from_numpy(x).contiguous().float()
    return rotation_6d_to_matrix(x).numpy()

# TODO 
def normalize_rot_vec_sequence(vec_seq):
    norm_rot_vec = torch.norm(vec_seq, dim=-1, keepdim=True)
    min_norm = torch.min(norm_rot_vec, dim=0, keepdim=True).values
