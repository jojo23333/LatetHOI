...
def solve_hand(cfg, vtemp, n_comps, T, h_params, is_right):
    with torch.no_grad():
    ...
    return full_pose.cpu().detach(), transl.cpu().detach(), verts.cpu().detach(), out.vertices.cpu().detach()
...
class GRABDataSet(object):
        ...
        for split in self.split_seqs.keys():
            ...
            for sequence in tqdm(seq_to_process):
                ...
                if cfg.save_lhand_verts:
                    ...
                    full_pose, l_trans, verts_lh, verts_lh_r = solve_hand(cfg, lh_vtemp, n_comps, T, lh_params, is_right=False)
                ...
                if cfg.save_rhand_verts:
                    ...
                    r_full_pose, r_trans, verts_rh, verts_rh_r = solve_hand(cfg, rh_vtemp, n_comps, T, rh_params, is_right=True)
                ...
                for i in range(0, T, 120):
                    if lhand_contact[i]:
                        ...
                        mesh = trimesh.Trimesh(vertices=verts_lh_r[i] - trans_obj[i].reshape(1, 3), faces=lh_m.faces, face_colors=name_to_rgb['gray'])
                        mesh.export(f'{path}/{i}_lhand_r.ply')
                        ...
                        mesh = trimesh.Trimesh(vertices=verts_rh_r[i] - trans_obj[i].reshape(1, 3), faces=rh_m.faces, face_colors=name_to_rgb['gray'])
                        ...
                        mesh = trimesh.points.PointCloud(vertices=verts_obj_mirror[i]- trans_obj_mirror[i].reshape(1, 3), vertex_colors=name_to_rgb['green'])
    ...
    def load_obj_verts(self, obj_name, seq_data, n_verts_sample=512):
        ...
        if obj_name not in self.obj_info:
            ...
            verts_obj = self.obj_info[obj_name]['verts']
            if verts_obj.shape[0] > n_verts_sample:
                verts_sample_id = np.random.choice(verts_obj.shape[0], n_verts_sample, replace=False)
            else:
                verts_sample_id = np.arange(verts_obj.shape[0])
            verts_sampled = verts_obj[verts_sample_id]
            self.obj_info[obj_name]['verts_sample'] = verts_sampled
            self.obj_info[obj_name]['verts_sample_id'] = verts_sample_id
...
if __name__ == '__main__':
    ...
    out_path = "../../data/grab/grab_grabnet_diffh2o"
    ...
    # grab_splits = { 
    #     'test': ['apple', 'mug', 'train', 'elephant', 'alarm clock', 'small pyramid', 'medium cylinder', 'large torus'],
    #     'val': [],
    #     'train': []
    # }
    grab_splits = { 
