...
class DexYCBMotion(data.Dataset):    
    ...
    def compute_clip_embedding(self):
        ...
        for seq_name in self.seqs:
            obj = np.load(seq_name, allow_pickle=True).item()['object']
            obj = ' '.join(obj.split('_')[1:])
            txt = f'pick up {obj}'
    ...
    def compute_canon_bps(self):
        ...
        for obj_mesh in os.listdir(mesh_path):
            mesh_file = os.path.join(mesh_path, obj_mesh, 'textured_simple.obj')
            print("Preprocessing: ", obj_mesh)
            verts_obj = trimesh.load(mesh_file).vertices
...
class DexYCBLatentMotion(DexYCBMotion):
...
class DexYCBMotionMLD(DexYCBLatentMotion):
    def __init__(self, root, split='train', max_frame=256):
        self.root = root
        self.split = split
        d = np.load(os.path.join(root, 'mean_std.npy'), allow_pickle=True).item()
...
def dexycb_motion(cfg, split):
    dataset = DexYCBMotion("data/dexYCB/dexycb_seq", split=split, max_frame=96)
    return dataset

@DATASET_REGISTRY.register()
def dexycb_latent_motion(cfg, split):
    dataset = DexYCBLatentMotion("data/dexYCB/dexycb_seq", split=split, max_frame=96)
    return dataset

@DATASET_REGISTRY.register()
def dexycb_motion_mld(cfg, split):
    dataset = DexYCBMotionMLD("data/dexYCB/dexycb_seq", split=split, max_frame=96)
    return dataset

if __name__ == '__main__':
    dataset = DexYCBMotion("data/dexYCB/dexycb_seq", split='train')
    print(dataset[0])
