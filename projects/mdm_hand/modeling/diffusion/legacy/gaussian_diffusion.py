...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, condition=None, model_kwargs=None, noise=None, dataset=None):
        """
        ...
        bsz = list(condition.values())[0].shape[0]
        mask_d = model.get_loss_mask(x_start, condition)
        assert isinstance(x_start, dict)
        x_start_d = x_start
        x_start = model.dict_2_tensor(x_start, condition)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output = model(x_t, scaled_t, condition=condition, is_train=True, **model_kwargs)
            if isinstance(model_output, tuple):
                if len(model_output) == 2:
                    model_output, pcd_trans = model_output
                else:
                    model_output, pcd_trans, wrist_pos = model_output
            target_d = model.tensor_2_dict(x_start, condition)
            output_d = model.tensor_2_dict(model_output, condition)

            if self.cfg.DIFFUSION.ANGLE_LOSS > 0:
                if pcd_trans.isnan().any():
                    ...
                    },f"DEBUG_{comm.get_local_rank()}.pth")
                    raise AssertionError("NAN in FEAT")
                bsz, nf = model_output.shape[:2]
                pcd_pos = torch.cat([condition["pcd_top_pos"], condition["pcd_bottom_pos"]], dim=-2).view(bsz, nf, 1, -1, 3)
                gt_relative_dis = torch.cat(
                    [x_start_d["pos_left_g"], x_start_d["pos_left_j"], x_start_d["pos_right_g"], x_start_d["pos_right_j"]],
                    dim=-1
                ).view(bsz, nf, 42, 1, 3) - pcd_pos
                pred_relative_dis = pcd_trans - pcd_pos
                def angle_v2(a, b, dim=-1, eps=0.0005):
                    a_norm = a.norm(dim=dim, keepdim=True)
                    b_norm = b.norm(dim=dim, keepdim=True)
                    numerator = (a * b_norm - a_norm * b).norm(dim=dim)
                    denominator = (a * b_norm + a_norm * b).norm(dim=dim)
                    ...
                    angle = 2 * torch.atan2(numerator, denominator)
                    # angle = angle * (a_norm.squeeze() > eps).to(angle)
                    # angle = angle * (b_norm.squeeze() > eps).to(angle)
                    return angle
                # For debugging
                # self.gt_relative_dis = gt_relative_dis
                # self.pred_relative_dis = pred_relative_dis
                angle_diff = angle_v2(gt_relative_dis, pred_relative_dis) / np.pi
                angle_diff_loss = (angle_diff - 0.).clip(min=0)
                # angle_diff_loss = torch.nan_to_num(angle_diff_loss, 0)
                losses[f'pos_angle_loss'] = angle_diff_loss.mean() * self.cfg.DIFFUSION.ANGLE_LOSS

            if self.cfg.MODEL.CONTACT_MASK_STD > 0:
                loss = self.masked_l2(target_d['pos_left_g'], wrist_pos[:,:,0,:]) * weights
                losses[f'left_wrist_coarse'] =  loss.mean()
                loss = self.masked_l2(target_d['pos_right_g'], wrist_pos[:,:,1,:]) * weights
                losses[f'right_wrist_coarse'] =  loss.mean()
