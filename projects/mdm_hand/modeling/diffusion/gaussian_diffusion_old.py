...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        self.schedule_sampler = create_named_schedule_sampler(self.schedule_sampler_type, self)
    ...
    def training_losses(self, model, x_dict, y=None, noise=None, dataset=None):
        """
        Compute training losses for a single timestep.

        :param model: the model to evaluate loss on.
        :param x_start: a dict of [N x C x ...] tensor of inputs.
        ...
        bsz = list(x_dict.values())[0].shape[0]
        shape_dict = {k: v.shape[-1] for k, v in x_dict.items()}
        
        def get_xdict(x, shape_dict):
            start = 0
            x_dict = {}
            for k, v in shape_dict.items():
                end = start + v
                if k == 'z':
                    assert start == 0 and end == 32
                x_dict[k] = x[..., start:end]
                start = end
            return x_dict
        
        # Get x_start tensor from input dict
        x_start = torch.cat([v for k, v in x_dict.items()], dim=-1)
        if self.cfg.AMP_ENABLED:
            x_start = x_start.to(torch.float16)
        else:
            x_start = x_start.to(torch.float32)
        t, weights = self.schedule_sampler.sample(bsz, x_start.device)

        # Get Noisy x_t
        if noise is None:
            noise = th.randn_like(x_start)
        x_t = self.q_sample(x_start, t, noise=noise).to(x_start)
        
        # Caluculate per input l2 loss
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            losses = {}
            scaled_t = self._scale_timesteps(t)
            model_output = model(x_t, scaled_t, y=y)
            
            target_d = x_dict
            output_d = get_xdict(model_output, shape_dict)
            mask = y['mask'] if 'mask' in y else None
            for k in target_d.keys():
                assert k in output_d
                assert target_d[k].shape == output_d[k].shape
                loss = self.masked_l2(target_d[k], output_d[k], mask) * weights
                losses[f'{k}_loss'] = loss.mean()# mean_flat(rot_mse)
        else:
        ...
        # Do decoding
        if 'vae' in y.keys():
            assert 'dec_fn' in y.keys()
            drec_all = y['dec_fn'](output_d['z'], output_d['feat'], y)

            # TODO calculate loss for left and right hand saperately
            # TODO add corresponding label in y as well ("verts_rhand" for left and right, as well as verts_object)
            # TODO NOTE that for lhand check use the unmirrored version.
            diff_coef = 0.05
            for k in ['lhand', 'rhand']:
                dorig, hand_model = y[k]
                drec = drec_all[k]
                bsz, nf, _ = drec['transl'].shape
                device = drec['transl'].device
                verts_object_gt = dorig['verts_object'].view(bsz*nf, -1, 3).to(drec['transl'])
                verts_rhand_gt = dorig['verts_rhand'].view(bsz*nf, -1, 3).to(drec['transl'])

                out_put = hand_model(
                    global_orient = drec['global_orient'].view(bsz*nf, -1),
                    hand_pose = drec['hand_pose'].view(bsz*nf, -1),
                    transl = drec['transl'].view(bsz*nf, -1),
                )
                verts_rhand = out_put.vertices

                ######### dist loss
                # TODO comment out dist losss for speed now
                hand_model_faces = torch.from_numpy(hand_model.faces.astype(np.int32)).view(1, -1, 3).repeat(bsz*nf, 1, 1).to(verts_rhand.device)
                rh_mesh = Meshes(verts=verts_rhand, faces=hand_model_faces).to(device).verts_normals_packed().view(-1, 778, 3)
                rh_mesh_gt = Meshes(verts=verts_rhand_gt, faces=hand_model_faces).to(device).verts_normals_packed().view(-1, 778, 3)

                o2h_signed, h2o, _ = point2point_signed(verts_rhand, verts_object_gt, rh_mesh)
                o2h_signed_gt, h2o_gt, o2h_idx = point2point_signed(verts_rhand_gt, verts_object_gt, rh_mesh_gt)

                # addaptive weight for penetration and contact verts
                w_dist = (o2h_signed_gt < 0.01) * (o2h_signed_gt > -0.005)
                w_dist_neg = o2h_signed < 0.
                w = torch.ones([bsz*nf, verts_object_gt.shape[1]]).to(verts_object_gt)
                w[~w_dist] = .1 # less weight for far away vertices
                w[w_dist_neg] = 1.5 # more weight for penetration
                
                loss_dist_h = 35 * diff_coef * torch.mean(torch.einsum('ij,j->ij', torch.abs(h2o.abs() - h2o_gt.abs()), y['v_weights2']))
                loss_dist_o = 30 * diff_coef * torch.mean(torch.einsum('ij,ij->ij', torch.abs(o2h_signed - o2h_signed_gt), w))
                ########## verts loss
                loss_mesh_rec_w = 35 * diff_coef * torch.mean(torch.einsum('ijk,j->ijk', torch.abs((verts_rhand_gt - verts_rhand)), y['v_weights']))
                ########## edge loss
                loss_edge = 30 * diff_coef * torch.nn.functional.l1_loss(edges_for(verts_rhand, y['vpe']), edges_for(verts_rhand_gt, y['vpe']))
                losses[f'{k}_loss_dist_h'] = loss_dist_h
                losses[f'{k}_loss_dist_o'] = loss_dist_o
                losses[f'{k}_loss_mesh_rec_w'] = loss_mesh_rec_w
                losses[f'{k}_loss_edge'] = loss_edge
