    ...
    def training_losses(self, model, x_dict, y=None, noise=None, dataset=None):
        ...
        def get_xdict(x, shape_dict):
            ...
            for k, v in shape_dict.items():
                ...
                if k == 'z':
                    assert start == 0 and (end == 32 or end == 16 or end == 64)
        ...
        if 'vae' in y.keys():
            ...
            for k in ['lhand', 'rhand']:
                ...
                verts_rhand = out_put.vertices
                loss_mesh = self.LossL2(verts_rhand, verts_rhand_gt).mean()
                # gt_rot_mat = torch.cat(
                #     [dorig['global_orient_rhand_rotmat'].unsqueeze(1).reshape(bsz*nf, 1, 3, 3), 
                #      dorig['fpose_rhand_rotmat'].reshape(bsz*nf, 15, 3, 3)], dim=1
                # ).view(-1, 16, 3, 3)
                # rot_mat_loss = self.LossL2(gt_rot_mat, drec['fullpose'].reshape(-1, 16, 3, 3))

                losses[f'{k}_loss_mesh'] = loss_mesh * 500
                # losses[f'{k}_loss_rot_mat'] = rot_mat_loss * 10
        return losses
