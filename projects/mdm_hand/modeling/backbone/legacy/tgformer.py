# Temporal Graph Transformer inspired from https://github.com/Walter0807/MotionBERT/blob/main/lib/model/DSTformer.py
...
class TGFormer(nn.Module):
    def __init__(self, dim_in=3, dim_out=3, dim_feat=256, dim_rep=512,
                 depth=5, num_heads=8, mlp_ratio=4, condition_dim=0,
                 num_joints=42, maxlen=243, 
        ...
        self.embed_timestep = TimestepEmbedder(dim_feat)
        if condition_dim > 0:
            self.condition_embed = nn.Sequential(
                nn.Linear(condition_dim, dim_feat),
                nn.SiLU(),
    ...
    def forward(self, x, timesteps, condition=None, return_rep=False, **kwargs):
        B, F, J, C = x.shape
        time_emb = self.embed_timestep(timesteps)
        time_emb = time_emb.view(B, 1, 1, -1).repeat(1, 1, J, 1)
        ...
        if condition is not None:
            if condition.ndim == 2:
                ...
                cond = self.condition_embed(condition).unsqueeze(2)
                x = x + cond

        x = torch.cat([time_emb, x], dim=1)
        x = x.reshape(B*(F+1), J, C)
        x = self.pos_drop(x)
        alphas = []
