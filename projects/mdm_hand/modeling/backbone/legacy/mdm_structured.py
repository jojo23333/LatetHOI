...
class STEncoder(nn.Module):
    ...
    def __init__(self, spatial_layer, temporal_layer, num_layers, norm=None, enable_nested_tensor=False):
    ...
    def forward(self, 
        ...
        for spatial_layer, temporal_layer in zip(self.spatial_layers, self.temporal_layers):
            ...
            output = output.transpose(2, 3).reshape(-1, 21, hidden_dim)
            output = output + (diff_cond["obj_shape_cond"] if "obj_shape_cond" in diff_cond else 0)
            output = spatial_layer(output, src_mask=mask)
            out_skip = output.view(bsz, 2, nf, 21, -1).transpose(2, 3)
            # Temporal for wrist only
            # [bsz, 2, 1, nf, hidden_dim]
            #             ↑↑
            # Operate on motion dimension
            # output = out_skip[:, :, 0].reshape(-1, nf, hidden_dim)
            output = torch.mean(out_skip, dim=2)
            output = output + seq_time_emb.view(1, 1, nf, -1)
            output = output + hand_emb.view(1, 2, 1, -1)
            output = output + diff_cond["obj_motion_cond"].view(bsz, 1, nf, -1)
            output = output.view(bsz, 2*nf, -1)
            # output = temporal_layer(output).view(bsz, 2, nf, -1)
            # out_skip += output
            output = temporal_layer(output).view(bsz, 2, 1, nf, -1)
            output = output + out_skip

        if self.norm is not None:
            output = self.norm(output)
        output = output.view(bsz, nhands, nj, nf, -1)
        return output

class STHandformer(MDMHandMANOCO):
    def __init__(self, *args, **kwargs):
        ...
        spatial_layer = nn.TransformerEncoderLayer(d_model=self.latent_dim,
                                                    ...
                                                    activation=self.activation,
                                                    batch_first=True)
        self.seqTransEncoder = STEncoder(spatial_layer, temporal_layer, num_layers=self.num_layers)
    ...
    def output(self, x):
        # x (bsz, 2, 21, nf, hidden_dim)
        bsz, _, _, nf, hidden_dim = x.shape
        joint = self.readout_joints(x[:, :, 1:])
        wrist = self.readout_wrist(x[:, :, :1])
        x = torch.cat([wrist, joint], dim=2).permute(0, 3, 1, 2, 4).contiguous()
    ...
    def forward(self, x, timesteps, condition, y=None, **kwargs):
        ...
        joint_emb = self.joint_embedding.weight.view(1, 1, 21, 1, -1)
        hand_emb = self.hand_embedding.weight.view(1, 2, 1, 1, -1)
        # hand_emb = hand_emb + joint_emb

        seq_time_emb = self.sequence_pos_encoder.pe[:nf, :].view(1, nf, hidden_dim)
        ...
        if len(condition.keys()) > 0:
            ...
            if 'obj_type' in condition:
                obj_tensor = self.obj_embedding[condition['obj_type']].view(bsz, 1, -1) # 1, bs, dim
                diff_cond["obj_shape_cond"] = obj_tensor

        out = self.seqTransEncoder(xseq, hand_emb, joint_emb, seq_time_emb, diff_cond)
        out = self.output(out)
        # TODO add output
        return out
...
def mdm_structured(cfg, input_shape):
    return STHandformer(input_dim=444, condition_dim=10, latent_dim=256, ff_size=512, num_layers=10,
