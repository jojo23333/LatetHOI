...
class STEncoder(nn.Module):
    ...
    def forward(self, 
        ...
        for spatial_layer in self.spatial_layers[num_encoder_layer:]:
            output = spatial_layer(output, src_mask=mask)
        output = output.view(bsz, nf, 2, 21, -1)
        wrist_pos = self.wrist_pos(output[...,0,:])
        ...
        return output, torch.cat([f[:, 0], f[:, 1]], dim=-2), wrist_pos
...
class STHandformer2(MDMHandMANOCO):
    ...
    def output(self, output, pcd_feat, pcd_pos, wrist_pos):
        ...
        if self.contact_mask_std > 0:
                # training time
            gt_dist = pcd_pos.view(bsz, nf, 1, n_pcd, 3) - wrist_pos.view(bsz, nf, 2, 1, 3).detach()
            gt_dist = torch.linalg.norm(gt_dist, dim=-1, keepdim=True)
            contact_mask = self.gaussian_density(gt_dist)
            contact_mask = (contact_mask + 1e-6) / (contact_mask.sum(dim=-2, keepdim=True) + n_pcd*1e-6)  # (bsz, nf, 2, n_pcd, 1)
            if contact_mask.isnan().any():
                print(gt_dist)
                print(contact_mask)
                assert False
            if pcd_trans.isnan().any():
                print(pcd_trans.isnan().sum(), "Value is NAN")
                pcd_trans = torch.nan_to_num(pcd_trans, 0, 0)

            contact_mask = contact_mask.view(bsz, nf, 2, 1, n_pcd).repeat(1, 1, 1, 21, 1).view(bsz, nf, 42, n_pcd, 1)
    ...
    def forward(self, x, timesteps, condition, y=None, is_train=False, x_target=None, **kwargs):
        ...
        out, pcd_feat, wrist_pos = self.st_encoder(xseq, pcd, diff_cond, condition)
        ...
        if is_train:
            return out, pcd_out, wrist_pos
        else:
            return out


@BACKBONE_REGISTRY.register()
def pcd_structured_decoder_focal(cfg, input_shape):
