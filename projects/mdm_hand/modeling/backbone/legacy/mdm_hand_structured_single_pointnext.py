...
class STEncoder(nn.Module):
    ...
    def __init__(self, latent_dim, spatial_layer, temporal_layer, num_layers, norm=None, enable_nested_tensor=False):
        ...
        self.latent_2_pcd_feat = nn.Sequential(
            nn.Linear(2*latent_dim, 2*latent_dim),
            nn.SiLU(),
            nn.Linear(2*latent_dim, 1024)
        )

        self.pointnext = get_pointnext(freeze_encoder=False)
    ...
    def forward(self, 
        ...
        f[-1] = f[-1] + self.latent_2_pcd_feat(seq[:, :1, :]).repeat(2, 1, 1).reshape(2*bsz, -1, 1)
        f = self.pointnext.decoder(p, f)
        fdim = f.shape[1]
        f = f.transpose(-1, -2).view(2, bsz, 1, -1, fdim).repeat(1, 1, nf , 1, 1)
        arti_embedding = self.pcd_arti_embedding(condition["obj_arti"].to(output)).view(bsz, nf, 1, 64)
        f = torch.cat(
            [f[0] + arti_embedding, f[1]],
            dim=-2
        ).view(bsz, nf, -1, fdim)
...
def pcd_structured_single_decoder(cfg, input_shape):
