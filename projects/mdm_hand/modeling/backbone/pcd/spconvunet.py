...
class SpUNetBase(nn.Module):
    ...
    def forward(self, feat, discrete_coord, offset):
...
class SpUNetEncoder(nn.Module):
    ...
    def forward(self, input):
        feat = input["feat"]
        offset = input["offset"]
        discrete_coord = input["discrete_coord"]
...
class SpUNetDecoder(nn.Module):
    ...
    def forward(self, feat, x, skips):
        batch = x.indices[:, 0].to(torch.int64)
        x = x.replace_feature(x.features + feat[batch, :])
        # dec forward
        for s in reversed(range(self.num_stages)):
            x = self.up[s](x)
            skip = skips.pop(-1)
