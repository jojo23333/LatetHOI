...
class MDM_UNET(nn.Module):
    ...
    def __init__(self,
        ...
        self.embed_bps = nn.Linear(4096, self.latent_dim)
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if 'bps' in y.keys():
            enc_obj = y['bps']
            emb += self.embed_bps(self.mask_cond(enc_obj, force_mask=force_mask))
        ...
        # [224, bs, d] (right pad)
        # assert nframes == 256, f"the input should be 256 frames not {nframes}"
...
def mdm_unet(cfg, input_shape=None):
    return MDM_UNET(
        nfeats=cfg.MODEL.INPUT_DIM, 
        latent_dim=cfg.MODEL.UNET.LATENT_DIM,
        dim_mults=cfg.MODEL.UNET.DIM_MULTS,
        adagn=cfg.MODEL.UNET.ADAGN,
    )